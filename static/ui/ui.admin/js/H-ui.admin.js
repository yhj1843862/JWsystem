...
function openPage(obj) {
	...
	var w = obj.getAttribute('data-width');
	w = w == null ? 800 : w;
...
function countStringLength(obj,min,max)
{
	var o  = $(obj);
	var c = o.val();
	var l = c.length;
	if(l > max)
	{
        ...
        o.val(c.substr(0,max));
        o.next().children().eq(0).html(max);
	}
	if(l < min)
	{
        o.next().css('color','red')
        o.next().children().eq(0).html(l);
	}
	if(l >= min && l <= max)
	{
        o.next().css('color','');
