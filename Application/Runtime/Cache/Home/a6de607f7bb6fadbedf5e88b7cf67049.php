            ...
            <div class="formControls col-xs-6 col-sm-6">
                <span class="select-box">
                    <select class="select" id="depart">
                        ...
                        <?php if(is_array($d_list)): $i = 0; $__LIST__ = $d_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["department_id"]); ?>"><?php echo ($v["department_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
				</span>
            </div>
        </div>
...
<script>
    $(function () {
        ...
        $('#depart').change(function () {
            //获取对应院系下的专业
            var did = $(this).val();
            $.post('<?php echo U("Department/ajax_profession_list_html");?>',{id:did},function (e) {
        ...
        $('#profess').change(function () {
            //获取专业下的班级信息
            var pid = $(this).val();
            $.post('<?php echo U("Profession/ajax_class_list");?>',{pid:pid},function (e) {
